...
class Cart:
    ...
    def add(self, product, quantity=1, override_quantity=False, size=None):
        ...
        # ПРОВЕРКА ДО ДОБАВЛЕНИЯ: Если новый товар и лимит уникальных позиций достигнут — НЕ ДОБАВЛЯЕМ (без очистки корзины)
        if unique_key not in self.cart and len(self.cart) >= MAX_CART_SIZE:
            messages.add_message(self.request, messages.WARNING, f"Лимит уникальных позиций ({MAX_CART_SIZE}) достигнут. Новый товар '{product.title}' не добавлен.")
            return
