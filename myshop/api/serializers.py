...
#             return []  # Placeholder для изображений, если product_price удалён
#         try:
#             request = self.context.get('request')
#             images = []
...
#             # Если продукт удалён (несмотря на существующий product_price)
#             return []

#     def get_product_zacup_price(self, obj):
#         if not obj.product_price:
#             return obj.zacup_price_snapshot  # Используем snapshot
#         try:
#             return obj.product_price.zacup_price
#         except AttributeError:
#             return obj.zacup_price_snapshot  # Fallback на snapshot

#     def get_product_mesto(self, obj):
#         if not obj.product_price:
#             return obj.mesto_snapshot  # Используем snapshot
#         try:
#             return obj.product_price.product.mesto
#         except (Product.DoesNotExist, AttributeError):
#             return obj.mesto_snapshot  # Fallback на snapshot

#     # Удалён to_representation: методы теперь обрабатывают None самостоятельно
...
#             if product_price.hidden:  # Пример: если есть поле hidden
...
#               else (item.product_price.zakup_price if item.product_price and item.product_price.zakup_price is not None else 0)) 
#              * item.quantity)
